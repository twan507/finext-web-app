...
- **② Danh mục Phòng Thủ / ③ Danh mục Mạo Hiểm (layout "AI Briefing"):** `BasketAiHero` (nhận định `stock_cmt` + 4 KPI) → curve rổ (`BasketPerformanceChart`) → `HoldingsTable` (holdings + lãi/lỗ từng mã từ vị thế mở `phase_trading`, `held!==1` lọc sang RankTable) → **[`RankTable` chờ vào | `OrderBook` sổ lệnh]** cạnh nhau trên màn rộng.
- **④ Sóng Ngành (CORE):** layout như ②③, thêm `IndustrySection` (rank ngành + heatmap `phase_industry` — chỉ render ngành từng có tín hiệu + `sector_cmt`) **ở CUỐI** (trước đây ở đầu — dời xuống để hero AI + curve lên trên).
