...
- **Kỹ thuật:** đổi từ N `AreaSeries` → **1 LineSeries trong suốt + `PhaseNeonPrimitive`** (custom `ISeriesPrimitive`, file mới `phaseChartPrimitive.ts`) vẽ toàn bộ visual bằng canvas (wash/drop-line/neon 3 lớp/glyph chip). Pulse dot + chip pha hiện tại = **overlay HTML kính mờ** neo tại điểm cuối (tính `timeToCoordinate`/`priceToCoordinate` sau fit + resize). Vạch giá cuối = `createPriceLine` màu pha. Tooltip nâng nền `backdropFilter blur`. Theme-aware (alpha dịu ở light). `tsc --noEmit` = 0 lỗi.
