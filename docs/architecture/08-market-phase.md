...
> **Cập nhật:** 2026-07-10 (tab rổ PAID: lookback 20 phiên + HoldingsTable 3 cấu hình cột — xem §8) · **Spec gốc:** [`../superpowers/specs/2026-07-09-market-phase-page-design.md`](../superpowers/specs/2026-07-09-market-phase-page-design.md)
...
- **Skeleton loading:** `components/MarketPhaseSkeleton.tsx` export `PhaseHeroSkeleton` (mirror hero 3 cột) + `MarketPhaseTabSkeleton` (mirror 4 section Tab ①: chart 300, chart 345 + panel, KPI+chart 320, 3 bảng top lệnh). MUI `Skeleton` bám sát layout. **2 giai đoạn loading:** (1) **auth-loading** — `OptionalAuthWrapper` nhận prop mới `loadingFallback` (thay `DefaultAuthSkeleton` generic); `PageContent` truyền vào `PhaseHeroSkeleton + TabSlider + MarketPhaseTabSkeleton` để paint đầu tiên đã đúng khung. (2) **data-loading** — `SharedPhaseHeader` → `PhaseHeroSkeleton`, `PageContent` (tab market) → `MarketPhaseTabSkeleton` khi `mp.isLoading`. PAID tab tự lo loading (BasketTab).
...
- **Tab ②③④ (PAID) — layout hiện tại (2026-07-10, sau nhiều vòng chỉnh):** `components/BasketTab.tsx` thứ tự: title "Lịch sử Hiệu suất danh mục" + `BasketPerformanceChart` → `BasketAiHero` (nhận định, DƯỚI chart) → **title lớn "Vận hành danh mục" + `SessionStrip`** (thanh 20 ô phiên, cùng hàng) → `HoldingsTable` → **[`RankTable` | `OrderBook`]** (2 cột trên `lg`) → (CORE) `IndustrySection` (cuối, luôn phiên mới nhất).
  - **`SessionStrip.tsx` (lookback):** 20 ô phiên (cũ→mới, màu pha từ `phase_daily` — `useBasketData` fetch thêm), ô chọn glow, click → 3 bảng hiện snapshot phiên đó: Holdings = `phase_basket` phiên đó; Rank/heldRanks = `phase_rank` phiên đó; OrderBook = trades closed `exit_date <= phiên` (stats theo tập lọc). `selectedDate` = derived (`|| latestDate`). Hero/chart KHÔNG theo phiên chọn.
  - **HoldingsTable 3 cấu hình cột (`table-layout: fixed`; width đơn giản: cột Mã **cố định 36%**, các cột còn lại **chia đều `(64/nOther)%`** đặt trên hàng header; cột **Trạng thái căn phải** như các cột số → mép cột thẳng hàng, hết cảm giác "lệch"):** (A) hiện tại + đang giữ → **Mã · Giá mua · Giá hiện tại · Số phiên · Trạng thái · Lãi/lỗ**; (B) quá khứ + đang giữ (`!isLatest`) → **Mã · Giá mua · Số phiên · Trạng thái** (bỏ Giá hiện tại/Lãi-lỗ vì không có MTM); (C) cash/downtrend (không nắm giữ) → **Mã · Biến động giá 6T (`mom120`) · Thanh khoản (`vma60`)** (bỏ Tỷ trọng/Trạng thái). Downtrend còn hiện banner compact (thanh dọc 4px + text) thay dải KPI — xem §5. Bỏ hẳn cột Tỷ trọng (5.8% đồng đều, vô ích). Prop đổi `heldRanks`→**`ranks`** = `stockRanks` (toàn bộ mã phiên) để mã `book` (state C) lấy được vma60/mom120. Tên công ty ellipsis theo bề rộng cột Mã (bỏ maxWidth cứng). `BasketAiHero.tsx` = thẻ FINEXT AI full-width (glass + ambient glow theo màu rổ + borderLeft accent), render `stock_cmt` **thuần** (đã bỏ KPI pills). KPI chuyển xuống **header bảng `HoldingsTable`** (Stack stats giống OrderBook, chỉ khi holding): **Lãi/lỗ danh mục · Số mã nắm giữ · Số mã sắp ra (`status=vung_buffer`) · Số mã chờ vào (`status=ung_vien`)** — tính ở BasketTab (`HoldingStat[]`). OrderBook stats đã **bỏ "Đang giữ"** (chỉ Số lệnh · Tỷ lệ thắng · LN TB/lệnh). `PortfolioComment.tsx` giờ **chỉ dùng cho `sector_cmt`** trong IndustrySection. `HoldingsTable` đã **bỏ dải KPI** (chuyển lên hero). **Bảng (Holdings/Rank/OrderBook)** bọc `AmbientCard glowColor={accent} filled={false}` (nền trong suốt, chỉ viền mảnh + 1px accent line — prop `filled` mới; `filled=false` cũng tắt radial glow + shadow band để không phủ header). Theme global `MuiProvider` tô paper cho TableHead/Cell.head/TableRow → các bảng này **đè lại bằng sx trên `<Table>`**: `.MuiTableHead-root/.MuiTableCell-head/.MuiTableRow-root → transparent`, hover alpha nhẹ. Header cho wrap (cột co được, tránh trượt ngang). **OrderBook chỉ hiện lệnh ĐÃ ĐÓNG** (bỏ dòng vị thế mở; stats giữ số "Đang giữ") + **equal-height với RankTable** (grid stretch + flex chain qua prop `rootSx` mới của AmbientCard; tràn thì cuộn trong card; xs cap 420px). Tên `Hiệu suất danh mục` (chung); tên rổ ưu tiên FE (`PRODUCT_FALLBACK_NAME`) hơn `display_name_vi` backend.
...
- **② Danh mục Phòng Thủ / ③ Danh mục Mạo Hiểm (layout cuối):** title "Hiệu suất danh mục" + curve rổ (`BasketPerformanceChart`) → `BasketAiHero` "Nhận định danh mục" (`stock_cmt` thuần, **dưới** chart, KHÔNG KPI) → **title lớn "Vận hành danh mục" + `SessionStrip`** (20 ô phiên, cùng hàng) → `HoldingsTable` (holdings + lãi/lỗ từng mã từ vị thế mở `phase_trading`; `held!==1` lọc sang RankTable) → **[`RankTable` chờ vào | `OrderBook` sổ lệnh]** cạnh nhau (equal-height). 3 bảng dưới thanh strip đổi theo phiên chọn; hero+chart KHÔNG đổi. **Không** có tiêu đề riêng cho từng bảng (chỉ 1 tiêu đề "Vận hành danh mục").
- **④ Sóng Ngành (CORE):** layout như ②③, thêm `IndustrySection` (rank ngành + heatmap `phase_industry` — chỉ render ngành từng có tín hiệu + `sector_cmt`) **ở CUỐI** (không theo phiên chọn — luôn phiên mới nhất).
...
- **Restart backend** (nạp: projection `phase_daily` có `conf_flat` + **crud mới 2026-07-10**: `phase_basket` limit 60, `phase_rank` 20 phiên — chưa restart thì SessionStrip chỉ ~3-4 ô, bảng quá khứ trống) + **chạy lại pipeline** (ghi 12 cột mới + 7 indicator_key mới). Trước khi có data mới: `conf_flat` + comment 7 chỉ số hiện `—`/trống (fail-safe).
