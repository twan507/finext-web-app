...
- **Đã commit** (owner tự commit dọc đường; HEAD `6508c2e` 2026-07-12). UI **user tự test** (quy ước dự án: không dựng browser/Playwright; **không tự chạy `next build`** — chỉ verify bằng `tsc --noEmit`).
...
- **Restart backend** (nạp: projection `phase_daily` có `conf_flat` + **crud mới**: `phase_basket` limit 60, `phase_rank` 20 phiên, **`phase_comment` 4 đoạn**, **`index_map`** — chưa restart thì SessionStrip chỉ ~3-4 ô, bảng quá khứ trống, tên ngành fallback về mã, tab market thiếu 3 đoạn nhận định) + **chạy lại pipeline** (ghi 12 cột mới + 7 indicator_key mới). Trước khi có data mới: `conf_flat` + comment 7 chỉ số hiện `—`/trống (fail-safe).
- ~~Chưa commit~~ → **đã commit** trên `main` (owner tự commit; HEAD `6508c2e`).
