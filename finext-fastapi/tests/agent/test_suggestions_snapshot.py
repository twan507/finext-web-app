...
def test_lay_final_phase_cua_phien_moi_nhat():
    # phase_daily sort date TĂNG dần → phần tử cuối là phiên hiện tại.
    phase = [{"date": "2026-07-21", "phase_label": "sideway"}, {"date": "2026-07-22", "phase_label": "uptrend"}]
    snap = build_snapshot(phase, [], [])
    assert snap["phase"] == "xu hướng tăng"
...
def test_industries_la_tap_nganh_cua_ma_bien_dong():
    ...
    snap = build_snapshot([], [], rows)
    assert sorted(set(snap["industries_up"]) | set(snap["industries_down"])) == ["Công nghệ", "Thép"]
