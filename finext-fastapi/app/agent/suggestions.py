...
# Giọng khẩu ngữ — owner chê 2 lần (22/07/2026): "ai" cho cổ phiếu ("VIC với VHM ai giảm
# nặng hơn") và đuôi chat-chit ("nhờ đâu vậy?"). Giọng chuẩn là trung lập kiểu báo tài chính.
# Prompt đã dặn nhưng M3 vẫn lọt → chặn cứng các marker phổ biến. " ai " cần đệm space để
# không dính từ chứa "ai" ("tài", "hai"...); câu luôn kết thúc "?" nên đuôi khớp dạng "vậy?".
_COLLOQUIAL_PATTERNS = (" ai ", "vậy?", "nhỉ?", "nhé?", " à?", "hả?")
...
def validate_suggestions(raw: str, allowed_tickers: set[str]) -> list[str] | None:
    ...
    for item in items:
        ...
        if any(p in low for p in _TIMING_PATTERNS):
            return None
        if low.startswith("ai ") or any(p in low for p in _COLLOQUIAL_PATTERNS):
            return None
...
_SYS = (
    ...
    "NGƯỜI HỎI LÀ AI: một nhà đầu tư phổ thông, KHÔNG rành thuật ngữ chuyên sâu, vừa mở app "
    "thấy thị trường biến động và muốn hiểu chuyện gì đang xảy ra.\n\n"
    "QUY TẮC 1 — GIỌNG TRUNG LẬP, CHUYÊN NGHIỆP, DỄ HIỂU. Như tiêu đề báo tài chính viết "
    "dạng câu hỏi: không thuật ngữ trong nghề, cũng không suồng sã khẩu ngữ.\n"
    "  SAI:  'Vì sao thị trường chưa đổi pha giảm?'  (thuật ngữ trong nghề)\n"
    "  ĐÚNG: 'Vì sao thị trường giảm mạnh hôm nay?'\n"
    "  SAI:  'VIC với VHM ai giảm nặng hơn vậy?'  (khẩu ngữ)\n"
    "  ĐÚNG: 'VIC hay VHM đang chịu áp lực bán lớn hơn?'\n"
    "Cổ phiếu/nhóm ngành gọi bằng 'mã nào'/'nhóm nào', KHÔNG dùng 'ai'. Không đuôi khẩu ngữ "
    "'vậy', 'nhỉ', 'nhé', 'à', 'hả'.\n\n"
    ...
    "  ĐÚNG: 'Nhóm chứng khoán có là điểm sáng hôm nay?'\n\n"
