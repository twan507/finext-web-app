...
- Rule 2 (tin tức/sự kiện chỉ dùng từ dữ liệu `news_*`; không có thì nói thật, không bịa): system prompt mục 7
...
Chỉ sau khi user xác nhận, agent mới query và phân tích. Nếu user không phản hồi, agent nêu rõ giả định mình đang hiểu rồi mới phân tích, không đoán ngầm.
...
## Case 2 — Bịa tin tức/sự kiện quốc tế không có trong dữ liệu
...
### Chẩn đoán
- **Vi phạm Rule 2** (tin tức/sự kiện chỉ được dùng khi có trong dữ liệu `news_*`; ngoài đó thì nói thật là không có)
...
**Bước 2:** Nếu diễn biến quốc tế cụ thể KHÔNG có trong `news_*` của dữ liệu → nói thật là không có thông tin cập nhật, KHÔNG lấp bằng trí nhớ.

**Bước 3:** Trình bày, ghi RÕ nguồn — chỉ khẳng định phần CÓ trong dữ liệu:

> "Về giá dầu & thị trường quốc tế — theo dữ liệu hệ thống cập nhật [ngày]: Dầu Brent [X] USD/thùng, tăng/giảm [Y]% tuần. S&P 500 [Z] điểm.
> 
> Về diễn biến eo biển Hormuz cụ thể — phần này nằm ngoài dữ liệu Finext hiện có, mình không có thông tin cập nhật để khẳng định. Anh/chị nên kiểm nguồn tin quốc tế trực tiếp; mình không muốn đưa số liệu chưa xác minh.
> 
> Tác động tới chứng khoán VN qua nhóm dầu khí: [dẫn số liệu DB của PVS, PVD, GAS]..."

### Nguyên tắc rút ra
- Tin tức, sự kiện quốc tế, diễn biến vĩ mô hiện tại → CHỈ trả lời từ tin trong dữ liệu (`news_*`, `other_data`). Không có trong dữ liệu thì **nói thật là không có thông tin cập nhật**, KHÔNG trả lời từ trí nhớ, KHÔNG bịa.
- Mỗi tin phải kèm nguồn: "theo dữ liệu tin tức hệ thống ngày [DD/MM]" (không lộ tên collection).
...
- Chỉ gán % khi: (a) có base rate lịch sử, (b) có mô hình option pricing/Monte Carlo, (c) có consensus từ nguồn đáng tin cậy CÓ TRONG dữ liệu
...
**Option B:** Benchmark từ báo cáo phân tích CÓ TRONG dữ liệu (ví dụ `data_briefing` doc `news_report`, hoặc nội dung tin trong `news_*`), trích đúng nguồn có trong hệ thống:

> "EIB: NIM 2.4%. Theo báo cáo trong hệ thống [ngày], NIM trung bình nhóm NHTM niêm yết khoảng [X]%. EIB thấp hơn trung bình ngành."

**Option C (nếu không có benchmark trong dữ liệu):** Không đưa benchmark, chỉ đưa con số tuyệt đối:

> "EIB: NIM 2.4%, CASA 13%, LDR 1.03. Không đưa so sánh ngành do chưa có số benchmark trong dữ liệu hệ thống."

### Nguyên tắc rút ra
- Benchmark là loại claim dễ bịa nhất vì "nghe có vẻ hợp lý"
- Mỗi benchmark trong response phải chỉ ra được nguồn: dữ liệu hệ thống (DB) — không bịa từ trí nhớ
...
| "Theo tin mới nhất..." (không có trong dữ liệu) | Rule 2 | Chỉ dùng tin trong `news_*`; không có thì nói thật |
| "Xác suất [X]%..." | Rule 5 | Đổi sang ngôn ngữ định tính |
| "Ngân hàng tốt thường [benchmark]..." | Rule 1 | Query DB (`industry_finstats`); không có thì bỏ benchmark |
