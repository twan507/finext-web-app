...
**Projection không cần khai báo `_id`** — hệ thống tự loại `_id` khỏi mọi kết quả. **Collection lớn** (`stock_*`, `industry_*`, `news_*`, `market_phase_history`, `phase_trading`, `phase_perf`...) bắt buộc projection liệt kê field cụ thể (không đọc full document); collection nhỏ có thể bỏ projection để lấy cả doc.
...
projection: { "<field_1>": 1, "<field_2>": 1 }
```
...
```
# gọi db_find
// 52 điểm cuối ≈ 1 năm gần nhất (52 tuần/năm) — vừa ngân sách kết quả tool
collection: history_finratios_stock
filter: { "ticker": "HPG" }
projection: { "ticker": 1, "type": 1, "series": { "$slice": -52 } }
```

⚠ Đừng kéo `$slice` dài hơn qua `db_find`: `-104` trả ~44k ký tự, vượt ngân sách nên hệ thống tự lược bớt còn ~1 năm kèm ghi chú — tốn công mà không được thêm dữ liệu. Cần cửa sổ **2 năm+** cho phân vị (D6) → dùng `db_stats` với `range`, server tính trên **toàn** chuỗi, không bị cắt:

```
# gọi db_stats
// phân vị 2 năm: so "latest" (pe hiện tại) với median/p25/p75/p95 của cửa sổ
collection: history_finratios_stock
field: "series.pe"
ops: ["latest", "median", "p25", "p75", "p95", "count"]
filter: { "ticker": "HPG" }
range: { "from": "<hôm nay lùi 2 năm, dạng YYYY-MM-DD>" }
```

Rồi tính **phân vị** của `pe` hiện tại (chuỗi -52 cho cửa sổ 1 năm; các mốc db_stats cho cửa sổ dài). **Cách đọc + thang phân vị + luật phân rã giá-vs-lợi-nhuận + fail-soft: `agent_db_04` mục D6** (canonical — đừng tự chế ngưỡng ở đây).
...
```
# gọi db_find
// P/E toàn thị trường 1 năm gần nhất (xu hướng); thống kê cửa sổ dài hơn → db_stats (ghi chú cuối mục)
collection: history_finratios_industry
filter: { "industry_name": "Toàn bộ thị trường" }
projection: { "series": { "$slice": -52 } }
...
`<N>` thường 10-20 là đủ (tối đa 30 — `max_slice` của `stock_itd`).
