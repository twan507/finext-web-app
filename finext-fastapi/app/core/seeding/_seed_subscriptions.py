...
from app.core.config import ADMIN_EMAIL, MANAGER_EMAIL, BROKER_EMAIL_1, BROKER_EMAIL_2, BASIC_LICENSE_KEY
...
async def seed_subscriptions(
    ...
    if MANAGER_EMAIL and MANAGER_EMAIL in user_ids_map and "MANAGER" in license_ids_map:
        subs_to_create_config.append({"email": MANAGER_EMAIL, "license_key": "MANAGER"})
    ...
    for sub_config_item in subs_to_create_config:
        ...
        existing_specific_active_sub = await db.subscriptions.find_one(
            {"user_id": user_obj_id, "license_key": license_key, "is_active": True, "expiry_date": {"$gt": datetime.now(timezone.utc)}}
        )
    ...
    for email, user_id_str_from_map in user_ids_map.items():
        ...
        try:
            ...
            logger.error(
                f"Error during assign_free_subscription_if_needed for user {email} (ID: {user_id_str_from_map}): {e}", exc_info=True
            )
