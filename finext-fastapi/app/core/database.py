...
async def connect_to_mongo():
    ...
    try:
        ...
        if "user_db" in mongodb.dbs and mongodb.dbs["user_db"] is not None:
            ...
            await db.sessions.create_index("access_jti", unique=True, sparse=True)  # NEW: access token JTI
            await db.sessions.create_index("refresh_jti", unique=True, sparse=True)  # NEW: refresh token JTI
            ...
            )  # User cannot have two watchlists with the same name
