...
from app.crud.sessions import get_sessions_by_user_id, delete_session_by_id, get_all_sessions
...
async def delete_my_specific_session(
    ...
    # require_permission("session", "delete_own") đã kiểm tra quyền
    ...
    # Điều này đã được thực hiện trong require_permission("session", "delete_own")
    ...
    if not deleted:
        logger.warning(
            f"User {current_user.email} tried to delete session ID {session_id} (JTI: {jti_to_log}) but it was not found or failed to delete (possibly already deleted)."
        )
