...
async def update_user_info_endpoint(
    ...
    # Đánh dấu vô hiệu hóa do admin để user không tự kích hoạt lại bằng OTP/Google.
    # Chỉ set ở đây — rollback activation phía dưới cố ý KHÔNG set cờ này vì đó là
    # hoàn tác một lần kích hoạt hỏng, không phải quyết định của admin.
    if update_dict_for_crud.get("is_active") is False and target_user.is_active is True:
        update_dict_for_crud["deactivated_by_admin"] = True
    elif is_activation_transition:
        update_dict_for_crud["deactivated_by_admin"] = False
