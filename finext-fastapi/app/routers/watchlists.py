...
async def update_my_watchlist( # Đổi tên hàm
    ...
    try:
        updated_watchlist = await crud_watchlists.update_watchlist(
            db, watchlist_id=watchlist_id, user_id=current_user.id, watchlist_update_data=watchlist_data # type: ignore
        )
    except ValueError as ve: 
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(ve))
...
async def admin_read_all_watchlists(
    ...
    if not hasattr(crud_watchlists, 'get_all_watchlists_admin'):
        ...
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="Admin list watchlists feature not fully implemented in CRUD.")
...
@api_response_wrapper(
    default_success_message="Watchlist đã được xóa thành công bởi Admin.",
    success_status_code=status.HTTP_200_OK
)
async def admin_delete_watchlist(
    ...
    if not hasattr(crud_watchlists, 'delete_watchlist_by_admin'):
        ...
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="Admin delete watchlist feature not fully implemented in CRUD.")
