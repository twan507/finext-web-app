...
async def create_or_reactivate_broker_for_user(db: AsyncIOMotorDatabase, user_id: PyObjectId) -> Optional[BrokerInDB]:
    ...
    if existing_broker_record:
        logger.info(
            f"User {user_id} đã có bản ghi broker (ID: {existing_broker_record['_id']}, Code: {existing_broker_record['broker_code']}). Sẽ kích hoạt lại nếu cần."
        )
        if not existing_broker_record.get("is_active"):
            update_result = await brokers_collection.update_one(
                {"_id": existing_broker_record["_id"]}, {"$set": {"is_active": True, "updated_at": now}}
            )
        ...
        if user_doc_for_broker.get("referral_code") != existing_broker_record["broker_code"]:
            await users_collection.update_one(
                {"_id": user_obj_id}, {"$set": {"referral_code": existing_broker_record["broker_code"], "updated_at": now}}
            )
            logger.info(
                f"Đã cập nhật/gán lại referral_code '{existing_broker_record['broker_code']}' cho user {user_id} khi kích hoạt lại broker."
            )
        ...
        try:
            ...
            if result.inserted_id:
                ...
                await users_collection.update_one({"_id": user_obj_id}, {"$set": {"referral_code": broker_code, "updated_at": now}})
...
async def update_broker_status(db: AsyncIOMotorDatabase, broker_id_or_code: str, is_active: bool) -> Optional[BrokerInDB]:
    ...
    if update_result.matched_count > 0:
        ...
        if not is_active:
            # Nếu hủy kích hoạt broker, tìm tất cả user đang dùng broker_code này
            # và xóa referral_code của họ.
            # Đồng thời, xóa referral_code của chính user là broker đó.

            # Xóa referral_code của chính user là broker
            await users_collection.update_one({"_id": user_id_of_broker_obj}, {"$set": {"referral_code": None, "updated_at": now}})
            logger.info(
                f"Đã xóa referral_code của user (broker) {str(user_id_of_broker_obj)} khi hủy kích hoạt broker {broker_id_or_code}."
            )

            # Tìm và xóa referral_code của các user khác đang sử dụng broker_code này
            users_affected_update_result = await users_collection.update_many(
                {
                    "referral_code": broker_code_of_broker,
                    "_id": {"$ne": user_id_of_broker_obj},
                },  # Loại trừ chính broker user đã xử lý ở trên
                {"$set": {"referral_code": None, "updated_at": now}},
            )
            if users_affected_update_result.modified_count > 0:
                logger.info(
                    f"Đã xóa referral_code cho {users_affected_update_result.modified_count} user(s) khác đang sử dụng mã broker {broker_code_of_broker}."
                )
            else:
                logger.info(f"Không có user nào khác sử dụng mã broker {broker_code_of_broker} để xóa referral_code.")
        else:
            # Nếu kích hoạt lại broker, đảm bảo referral_code của user là broker_code của họ
            await users_collection.update_one(
                {"_id": user_id_of_broker_obj}, {"$set": {"referral_code": broker_code_of_broker, "updated_at": now}}
            )
            logger.info(
                f"Đã gán lại referral_code '{broker_code_of_broker}' cho user {str(user_id_of_broker_obj)} khi kích hoạt broker {broker_id_or_code}."
            )
