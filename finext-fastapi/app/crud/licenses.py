...
async def get_licenses(
    ...
    licenses_cursor = (
        db[LICENSES_COLLECTION].find(query)
...
async def update_license(db: AsyncIOMotorDatabase, license_id: PyObjectId, license_update_data: LicenseUpdate) -> Optional[LicenseInDB]:
    ...
    if existing_license.key in PROTECTED_LICENSE_KEYS:
        ...
        if protected_fields_attempted_to_change:
            logger.warning(f"Attempt to modify protected fields ({', '.join(protected_fields_attempted_to_change)}) of protected license '{existing_license.key}'. Denied.")
            raise ValueError(f"Không thể thay đổi các trường được bảo vệ ({', '.join(protected_fields_attempted_to_change)}) của license hệ thống '{existing_license.key}'.")
    ...
    updated_result = await db[LICENSES_COLLECTION].update_one(
...
async def deactivate_license_db(db: AsyncIOMotorDatabase, license_id: PyObjectId) -> Optional[LicenseInDB]:
    ...
    active_subscriptions_count = await db[SUBSCRIPTIONS_COLLECTION].count_documents(
    ...
    if active_subscriptions_count > 0:
        ...
        raise ValueError(f"License '{license_to_deactivate.key}' đang được sử dụng bởi {active_subscriptions_count} subscription đang hoạt động và không thể vô hiệu hóa.")
    ...
    if update_result.matched_count > 0: # Found but not modified (already inactive - should have been caught above)
        logger.warning(f"License {license_id} (key: {license_to_deactivate.key}) found but not modified during deactivation (already inactive?).")
        return await get_license_by_id(db, license_id)
