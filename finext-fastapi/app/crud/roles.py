...
async def create_role(db: AsyncIOMotorDatabase, role_create_data: RoleCreate) -> Optional[RoleInDB]:
    ...
    try:
        ...
        if insert_result.inserted_id:
            # Đồng bộ field 'roles' trong permissions
            await _update_permissions_roles_field(db, role_create_data.name, valid_permission_obj_ids)
...
async def update_role(db: AsyncIOMotorDatabase, role_id_str: PyObjectId, role_update_data: RoleUpdate) -> Optional[RoleInDB]:
    ...
    if (
        "name" in role_update_data.model_dump(exclude_unset=True)
        and existing_role.get("name") in PROTECTED_ROLE_NAMES
        and role_update_data.name != existing_role.get("name")
    ):
    ...
    if not update_data:  # Nếu không có trường nào hợp lệ để cập nhật
        return await get_role_by_id(db, role_id_str)  # Trả về role hiện tại    update_data["updated_at"] = datetime.now(timezone.utc)

    updated_result = await db[ROLES_COLLECTION].update_one({"_id": role_obj_id}, {"$set": update_data})

    if updated_result.matched_count > 0:
        # Đồng bộ lại trường 'roles' trong permissions nếu permission_ids có thay đổi
        if "permission_ids" in update_data:
            # Sử dụng tên role hiện tại (có thể đã được cập nhật hoặc giữ nguyên)
            current_role_name = update_data.get("name", existing_role.get("name"))
            await _update_permissions_roles_field(db, current_role_name, update_data["permission_ids"])
...
async def delete_role(db: AsyncIOMotorDatabase, role_id_str: PyObjectId) -> bool:
    ...
    if delete_result.deleted_count > 0:
        # Đồng bộ xóa role_name khỏi tất cả permissions khi xóa role
        await db[PERMISSIONS_COLLECTION].update_many(
            {"roles": role_name_to_delete}, {"$pull": {"roles": role_name_to_delete}, "$set": {"updated_at": datetime.now(timezone.utc)}}
        )
        logger.info(f"Đã xóa role '{role_name_to_delete}' khỏi tất cả permissions")
        return True
    return False


async def _update_permissions_roles_field(db: AsyncIOMotorDatabase, role_name: str, permission_ids: List[ObjectId]):
    """
    Cập nhật field 'roles' trong permissions khi role thay đổi permission_ids.
    Logic đơn giản:
    1. Xóa role_name khỏi tất cả permissions
    2. Thêm role_name vào các permissions mới được assign
    """
    try:
        # Bước 1: Xóa role_name khỏi tất cả permissions
        await db[PERMISSIONS_COLLECTION].update_many(
            {"roles": role_name}, {"$pull": {"roles": role_name}, "$set": {"updated_at": datetime.now(timezone.utc)}}
        )

        # Bước 2: Thêm role_name vào các permissions được assign
        if permission_ids:
            await db[PERMISSIONS_COLLECTION].update_many(
                {"_id": {"$in": permission_ids}}, {"$addToSet": {"roles": role_name}, "$set": {"updated_at": datetime.now(timezone.utc)}}
            )

        logger.info(f"Đã đồng bộ field 'roles' cho role '{role_name}' với {len(permission_ids)} permissions")

    except Exception as e:
        logger.error(f"Lỗi khi đồng bộ permissions cho role '{role_name}': {e}")
