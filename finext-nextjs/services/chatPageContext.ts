...
//  a. Giọng phân tích TRUNG LẬP, CHUYÊN NGHIỆP mà vẫn dễ hiểu (kiểu tiêu đề tài chính dạng câu hỏi):
//     NGẮN GỌN, bỏ chữ đệm thừa; KHÔNG chatty, KHÔNG đuôi "nhé/vậy/nhỉ".
//  b. Hỏi về THỨ NĐT quan tâm (giá, dòng tiền, định giá, xu hướng); tránh khung xin lệnh mua/bán
//     ("có nên mua") — hỏi khách quan ("đắt hay rẻ", "định giá đã hấp dẫn chưa").
//  c. KHÔNG hỏi về cách vận hành giao diện; KHÔNG dùng tên tab / quy ước nội bộ; KHÔNG chèn slug
//     đường dẫn (VD 'nganhang') — dùng "ngành này" / "nhóm này".
//  d. Mỗi trang và mỗi tab một kho riêng, tối thiểu ba câu, không lặp giữa các trang.
//  e. Chỉ hỏi thứ agent trả lời được từ dữ liệu — agent KHÔNG đọc được danh mục riêng của user.
//
...
export const BUBBLE_GREETINGS: string[] = [
  'Bạn muốn phân tích gì ở trang này?',
  'Mình có thể làm rõ dữ liệu nào cho bạn?',
  'Bạn quan tâm chỉ số hay cổ phiếu nào?',
  'Cần xem sâu phần nào, bạn cứ hỏi mình.',
  'Bạn muốn hiểu thêm điều gì trên trang này?',
  'Hỏi mình bất cứ điều gì về thị trường.',
];
...
type DynamicEntry = Entry & {
  ...
  prefix: string;
  /** Chủ thể là MÃ/CHỈ SỐ (ticker/index) → viết HOA. Để tắt cho slug ngành/nhóm (VD 'nganhang'). */
  upperSubject?: boolean;
};
...
const EXACT: Record<string, Entry> = {
  '/': {
    ...
    pool: () => [
      ...
      'Nhóm ngành nào tăng tốt nhất phiên này?',
      'Vì sao thị trường hôm nay tăng/giảm?',
      'Cổ phiếu nào tăng mạnh nhất hôm nay?',
  ...
  '/phase': {
    ...
    pool: byTab(
      [
        ...
        'Tỷ trọng nắm giữ gợi ý hiện là bao nhiêu?',
        'Giai đoạn hiện tại kéo dài bao lâu rồi?',
        'Điều gì khiến thị trường chuyển giai đoạn?',
        'Cường độ thị trường đang mạnh hay yếu?',
      ],
      {
        conservative: [
          'Rổ này đang nắm giữ những mã nào?',
          'Rổ này hợp với người ngại rủi ro không?',
          'Hiệu quả rổ này thời gian qua ra sao?',
          'Rổ này khác hai rổ còn lại thế nào?',
        ],
        aggressive: [
          'Rổ này đang có những mã nào?',
          'Rổ này rủi ro tới đâu?',
          'Hiệu quả rổ này thời gian qua ra sao?',
          'Rổ này hợp khẩu vị đầu tư nào?',
        ],
        core: [
          'Rổ này đang bám theo ngành nào?',
          'Rổ này đang nắm giữ những mã nào?',
          'Vì sao rổ chọn những ngành đó?',
          'Hiệu quả rổ này thời gian qua ra sao?',
        ],
      },
    ),
  ...
  '/stocks': {
    ...
    pool: () => [
      ...
      'Nên lọc cổ phiếu theo tiêu chí nào?',
      'Cổ phiếu nào vừa vượt đỉnh ngắn hạn?',
      'Mã nào đang khoẻ hơn thị trường?',
  ...
  '/sectors': {
    ...
    pool: () => [
      ...
      'Ngành nào có định giá hấp dẫn?',
      'Ngành nào hút tiền nhiều nhất tuần này?',
      'Giai đoạn này nên ưu tiên ngành nào?',
    ],
  },
  '/groups': {
    title: 'Nhóm cổ phiếu',
    ...
    pool: () => [
      ...
      'Vốn hoá lớn hay nhỏ đang khoẻ hơn?',
      'Các nhóm này khác ngành nghề thế nào?',
      'Nhóm nào đang chạy tốt hơn thị trường?',
      'Nhóm nào biến động mạnh nhất tuần này?',
    ],
  },
  '/commodities': {
    ...
    pool: byTab(
      ...
      ],
      {
        ...
        energy: [
          ...
          'Giá năng lượng cao có đẩy lạm phát không?',
        ],
        chemical: [
          'Giá hoá chất, phân bón đang ra sao?',
        ...
        agriculture: [
          ...
          'Ai hưởng lợi khi giá nông sản tăng?',
          'Giá gạo và cà phê đang ra sao?',
  ...
  '/macro': {
    ...
    pool: byTab(
      [
        ...
        'Bối cảnh vĩ mô đang thuận hay nghịch?',
      ],
      {
        economy: [
        ...
        monetary: [
          'Lãi suất hiện cao hay thấp?',
          'Lãi suất tác động thế nào tới chứng khoán?',
          'Tiền đang được bơm ra hay hút về?',
          'Lãi suất tiết kiệm đang ra sao?',
          'Lãi suất giảm thì nhóm nào hưởng lợi?',
        ],
        exchange_rate: [
          'Tỷ giá đang căng hay ổn định?',
          'Tỷ giá tăng ảnh hưởng nhóm cổ phiếu nào?',
          'Đồng Việt Nam mất giá bao nhiêu năm nay?',
          'Tỷ giá có đang gây áp lực lên thị trường?',
        ],
      },
    ),
  },
  '/international': {
    ...
    pool: byTab(
      ...
      {
        global_index: [
          'Chứng khoán Mỹ và châu Á đang thế nào?',
          'Thế giới giảm thì Việt Nam có bị kéo theo?',
        ...
        fx: [
          ...
          'Vì sao cần theo dõi tỷ giá quốc tế?',
        ],
        bonds: [
        ...
        crypto: [
          ...
          'Tiền mã hoá có liên quan gì tới chứng khoán?',
  ...
  '/watchlist': {
    ...
    pool: () => [
      ...
      'Nên theo dõi cổ phiếu theo tiêu chí nào?',
...
const DYNAMIC: DynamicEntry[] = [
  {
    prefix: '/stocks',
    upperSubject: true, // chủ thể là mã cổ phiếu
    ...
    pool: byTabWithSubject(
      (s) => [
        ...
        `${s} đang đắt hay rẻ?`,
        `${s} làm ăn có tốt không?`,
        `Định giá ${s} đã hấp dẫn chưa?`,
        `${s} có gì đáng chú ý gần đây?`,
      ],
      {
        cashflow: (s) => [
          `Ai đang mua bán ${s} nhiều nhất?`,
          `Dòng tiền vào ${s} mạnh hay yếu?`,
          `Khối ngoại đang gom hay xả ${s}?`,
          `Thanh khoản ${s} có bất thường không?`,
        ],
        pricemap: (s) => [
          `${s} đang ở xu hướng nào?`,
          `${s} có hỗ trợ, kháng cự ở đâu?`,
          `${s} đang trên hay dưới các đường trung bình?`,
          `Tín hiệu kỹ thuật của ${s} ra sao?`,
        ],
        financials: (s) => [
          `${s} làm ăn có lãi không?`,
          `Kết quả kinh doanh gần đây của ${s} ra sao?`,
          `${s} có vay nợ nhiều không?`,
          `Lợi nhuận ${s} tăng hay giảm so cùng kỳ?`,
        ],
        news: (s) => [
    ...
    pool: byTab(
      ...
      ],
      {
        ...
        news: [
          ...
          'Chính sách nào đang ảnh hưởng ngành này?',
  ...
  },
  {
    ...
    pool: () => [
      ...
      'Nhóm này khoẻ hay yếu hơn thị trường?',
      'Dòng tiền vào nhóm này ra sao?',
      'Nhóm này được xếp theo tiêu chí gì?',
      'Mã nào nổi bật nhất nhóm này?',
    ],
  },
  {
    prefix: '/charts',
    upperSubject: true, // chủ thể là mã/chỉ số (VD 'fnxindex' → 'FNXINDEX')
    ...
    pool: (s) => [
      ...
      `${s} đang nằm trên hay dưới các đường trung bình?`,
      `${s} có vùng hỗ trợ, kháng cự quanh đâu?`,
      `Tín hiệu kỹ thuật của ${s} đang tích cực hay tiêu cực?`,
      `${s} biến động thế nào trong một năm qua?`,
    ],
  },
];
...
function resolve(pathname: string): { entry: Entry; subject?: string } | undefined {
  ...
  for (const d of DYNAMIC) {
    ...
    return { entry: d, subject: d.upperSubject ? raw.toUpperCase() : raw };
  }
  return undefined;
}
