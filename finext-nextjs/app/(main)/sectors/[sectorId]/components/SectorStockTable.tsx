...
// ========== TYPES ==========
export interface SectorStockRowData {
    ticker: string;
    close: number;
    diff?: number;
    pct_change?: number;
...
interface SortConfig {
    key: keyof SectorStockRowData | null;
    direction: SortDirection;
}

// ========== COLUMN DEFINITION ==========
interface ColumnDef {
    key: keyof SectorStockRowData;
    label: string;
...
function HeaderCell({
    ...
    onSort: (key: keyof SectorStockRowData) => void;
}) {
    const theme = useTheme();
...
interface SectorStockTableProps {
    data: SectorStockRowData[];
    isLoading?: boolean;
    skeletonRows?: number;
}
...
export default function SectorStockTable({
    ...
    const handleSort = useCallback((key: keyof SectorStockRowData) => {
        setSortConfig((prev) => {
            if (prev.key !== key) {
