...
function clusterLevels(levels: PriceLevel[]): ConfluenceCluster[] {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (pctGap <= CONFLUENCE_THRESHOLD) {
            ...
            clusters.push({ levels: currentCluster, avgPrice: avg, avgPctDiff: avgPct, isAbove: avgPct > 0 });
...
export default function VNINDEXPriceMap({ chartIndicatorData, currentPrice, currentDiff, currentPctChange }: VNINDEXPriceMapProps) {
    ...
    const priceLevels = useMemo<PriceLevel[]>(() => {
        ...
        for (const def of ALL_FIELD_DEFS) {
            ...
            levels.push({ price: value, label: def.label, field: def.field, group: def.group, groupLabel: def.groupLabel, timeframe: def.timeframe, pctDiff, diff });
        }
        return levels;
    ...
    if (!chartIndicatorData || currentPrice <= 0) {
        return (
            <Box>
                <Box sx={{ ...getGlassCard(isDark), p: 2, borderRadius: `${borderRadius.lg}px` }}>
            ...
            <Box sx={{ ...getGlassCard(isDark), p: 2, borderRadius: `${borderRadius.lg}px` }}>
                ...
                <Box sx={{ mb: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, flexWrap: 'wrap' }}>
                        <Typography sx={{ fontSize: getResponsiveFontSize('xs'), color: 'text.secondary', fontWeight: fontWeight.medium, minWidth: 52 }}>
                            Khung:
                        </Typography>
                        {TIMEFRAMES.map(tf => {
                            ...
                            return (
                                <Chip
                                    ...
                                    sx={{
                                        ...
                                        '&:hover': { bgcolor: alpha(tfColor, 0.12) },
                                    }}
                                />
                            );
                        })}
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                    ...
                    {aboveClusters.length > 0 && (
                        <>
                            ...
                            >
                                <Typography sx={{ fontSize: getResponsiveFontSize('xs'), fontWeight: fontWeight.bold, color: isDark ? '#69F0AE' : '#2E7D32', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                    {resistanceCollapsed ? '▶' : '▲'} Kháng cự
                                </Typography>
                                <Box sx={{ flex: 1, height: '1px', bgcolor: alpha(isDark ? '#69F0AE' : '#2E7D32', 0.25) }} />
                                <Typography sx={{ fontSize: getResponsiveFontSize('xxs'), color: theme.palette.text.disabled, fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    <Box sx={{
                        ...
                        <Box sx={{ width: 10, height: 10, borderRadius: '50%', bgcolor: theme.palette.warning.main, boxShadow: `0 0 8px ${alpha(theme.palette.warning.main, 0.5)}`, flexShrink: 0 }} />
                        <Typography sx={{ fontSize: getResponsiveFontSize('md'), fontWeight: fontWeight.bold, color: theme.palette.warning.main, fontVariantNumeric: 'tabular-nums' }}>
                            {currentPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </Typography>
                        <Typography sx={{ fontSize: getResponsiveFontSize('sm'), fontWeight: fontWeight.semibold, color: theme.palette.warning.main }}>
                            Giá hiện tại
                        </Typography>
                        <Box sx={{ flex: 1 }} />
                    ...
                    {belowClusters.length > 0 && (
                        <>
                            ...
                            >
                                <Typography sx={{ fontSize: getResponsiveFontSize('xs'), fontWeight: fontWeight.bold, color: isDark ? '#FF5252' : '#C62828', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                    {supportCollapsed ? '▶' : '▼'} Hỗ trợ
                                </Typography>
                                <Box sx={{ flex: 1, height: '1px', bgcolor: alpha(isDark ? '#FF5252' : '#C62828', 0.25) }} />
                                <Typography sx={{ fontSize: getResponsiveFontSize('xxs'), color: theme.palette.text.disabled, fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    {priceLevels.length === 0 && (
                        <Typography color="text.secondary" sx={{ textAlign: 'center', py: 4, fontSize: getResponsiveFontSize('sm') }}>
...
function ClusterRow({ cluster, isDark, theme, isResistance, getTimeframeColor, getTimeframeLabel }: ClusterRowProps) {
    ...
    const zoneColor = isResistance ? (isDark ? '#69F0AE' : '#2E7D32') : (isDark ? '#FF5252' : '#C62828');

    return (
        <Box sx={{
            borderRadius: `${borderRadius.sm}px`,
            border: isConfluence ? `1px solid ${alpha(zoneColor, 0.25)}` : 'none',
            bgcolor: isConfluence ? alpha(zoneColor, 0.04) : 'transparent',
            mb: isConfluence ? 0.5 : 0,
            overflow: 'hidden',
        }}>
            {isConfluence && (
                <Box sx={{ px: 1, py: 0.25, bgcolor: alpha(zoneColor, 0.08), borderBottom: `1px solid ${alpha(zoneColor, 0.12)}`, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <Typography sx={{ fontSize: getResponsiveFontSize('xxs'), fontWeight: fontWeight.bold, color: zoneColor, letterSpacing: '0.3px' }}>
                ...
                .map((level, li) => (
                    <Box
                        ...
                        sx={{
                            ...
                            gap: { xs: 0.5, md: 1 },
                            px: 1.5, py: 0.6,
                            borderBottom: li < cluster.levels.length - 1 ? `1px solid ${alpha(theme.palette.divider, 0.08)}` : 'none',
                            '&:hover': { bgcolor: alpha(zoneColor, 0.06) },
                            transition: 'background 0.15s ease',
                        }}
                    >
                        <Typography sx={{ fontSize: getResponsiveFontSize('sm'), fontWeight: fontWeight.semibold, color: theme.palette.text.primary, fontVariantNumeric: 'tabular-nums' }}>
                            {formatPrice(level.price)}
                        </Typography>
                        <Typography sx={{ fontSize: getResponsiveFontSize('sm'), fontWeight: fontWeight.bold, color: getTimeframeColor(level.timeframe, level.field) }}>
                            {level.label}
                        </Typography>
                        <Typography sx={{ fontSize: getResponsiveFontSize('xxs'), fontWeight: fontWeight.medium, color: theme.palette.text.secondary, display: { xs: 'none', md: 'block' } }}>
                            {level.groupLabel}
                        </Typography>
                        <Box sx={{ display: { xs: 'none', md: 'flex' }, justifyContent: 'center' }}>
                            {level.timeframe ? (
                                <Typography sx={{ fontSize: getResponsiveFontSize('xxs'), fontWeight: fontWeight.bold, color: getTimeframeColor(level.timeframe), width: 22, textAlign: 'center' }}>
                                    {level.timeframe.toUpperCase()}
                                </Typography>
                            ) : (
                                <Typography sx={{ fontSize: getResponsiveFontSize('xxs'), color: theme.palette.text.disabled, width: 22, textAlign: 'center' }}>
                                    —
                                </Typography>
                            )}
                        </Box>
                        <Typography sx={{ fontSize: getResponsiveFontSize('xs'), fontWeight: fontWeight.semibold, color: zoneColor, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
                            {formatDiff(level.diff)}
                        </Typography>
                    </Box>
                ))}
        </Box>
