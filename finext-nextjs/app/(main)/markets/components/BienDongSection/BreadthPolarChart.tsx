...
export default function BreadthPolarChart({ series, labels, colors, chartHeight }: BreadthPolarChartProps) {
        ...
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            {!hasData ? (
                ...
                <>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mb: 4.8, flexWrap: 'wrap' }}>
                        {[1, 2, 3].map((i) => (
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mb: 4.8, flexWrap: 'wrap' }}>
                        {labels.map((label, index) => (
                            <Box key={label} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                ...
                                <Typography color="text.secondary" sx={{ fontSize: getResponsiveFontSize('sm').md, fontWeight: fontWeight.medium }}>
                                    {label}
                                </Typography>
                            </Box>
                        ))}
