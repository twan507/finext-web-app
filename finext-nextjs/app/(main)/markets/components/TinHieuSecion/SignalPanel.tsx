...
const SignalIcon = ({ type, size = 16 }: SignalIconProps) => {
    ...
    const iconStyles = { fontSize: size, flexShrink: 0 };

    switch (type) {
        case 'positive':
            return <CheckCircleIcon sx={{ ...iconStyles, color: theme.palette.trend.up }} />;
        case 'warning':
            return <WarningIcon sx={{ ...iconStyles, color: theme.palette.warning.main }} />;
        case 'negative':
            return <CancelIcon sx={{ ...iconStyles, color: theme.palette.trend.down }} />;
    }
};
        ...
        <Box ref={anchorRef}>
            <Box
                ...
                sx={{
                    ...
                    py: 0.7, pl: 1.5, pr: 1.5, borderRadius: 1,
                    cursor: 'pointer', flex: 1, userSelect: 'none',
                    bgcolor: open ? alpha(infoColor, 0.08) : 'transparent',
                }}
            >
                <SignalIcon type={iconType} size={18} />
                <Typography sx={{ fontSize: getResponsiveFontSize('md'), color: theme.palette.text.secondary, flex: 1, lineHeight: 0.5 }}>
                    {label}
                </Typography>
            </Box>
            <Popover
                ...
                disableRestoreFocus
                slotProps={{ paper: { sx: { bgcolor: theme.palette.background.default, borderRadius: `${borderRadius.sm}px`, boxShadow: theme.shadows[8], border: `1px solid ${alpha(infoColor, 0.25)}`, px: 1.5, py: 1, maxWidth: 260 } } }}
            >
                <Typography sx={{ fontSize: getResponsiveFontSize('sm'), color: infoColor, lineHeight: 1.5 }}>
                    {tooltipText}
                </Typography>
            </Popover>
        </Box>
...
const SignalTable = ({ title, signals, signalType }: SignalTableProps) => {
    const theme = useTheme();
    const headerColor = signalType === 'buy' ? theme.palette.trend.up : theme.palette.warning.main;

    return (
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 1.5, px: 1.5, borderBottom: `1px solid ${theme.palette.divider}`, bgcolor: alpha(headerColor, 0.08), borderRadius: `${borderRadius.xs}px ${borderRadius.xs}px 0 0` }}>
                <Typography sx={{ fontSize: getResponsiveFontSize('md'), fontWeight: fontWeight.semibold, color: headerColor }}>{title}</Typography>
            </Box>
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', py: 1, overflowY: 'auto', justifyContent: 'space-evenly' }}>
                {signals.map((signal) => (
                    <SignalRow key={signal.key} label={signal.label} value={signal.value} signalType={signalType}
...
export default function SignalPanel({ data, isLoading = false }: SignalPanelProps) {
    const theme = useTheme();
    const buySignals = buySignalKeys.map((key) => ({ key, label: conditionInfo[key] || key, value: (data?.[key] ?? 0) as 0 | 1 }));
    const sellSignals = sellSignalKeys.map((key) => ({ key, label: conditionInfo[key] || key, value: (data?.[key] ?? 0) as 0 | 1 }));

    if (isLoading || !data) {
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 1, opacity: 0.5 }}>
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 2, overflow: 'hidden' }}>
            <SignalTable title="Tín hiệu Tích cực" signals={buySignals} signalType="buy" />
            <SignalTable title="Tín hiệu Tiêu cực" signals={sellSignals} signalType="sell" />
        </Box>
    );
}
...
export function SignalPanelCompact({ data, isLoading = false }: SignalPanelCompactProps) {
    ...
    if (isLoading || !data) {
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, p: 1, opacity: 0.5 }}>
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 2, overflow: 'hidden' }}>
            {/* Buy Section */}
            <Box sx={{ display: 'flex', flexDirection: 'column', bgcolor: alpha(theme.palette.trend.up, 0.06), borderRadius: borderRadius.sm }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pt: 1.5, px: 1.5 }}>
                    <Typography sx={{ fontSize: getResponsiveFontSize('md'), fontWeight: fontWeight.semibold, color: theme.palette.trend.up }}>
                        Tín hiệu Tích cực
                    </Typography>
                    <Typography sx={{ fontSize: getResponsiveFontSize('md'), fontWeight: fontWeight.bold, color: theme.palette.trend.up }}>
                        {buyPositiveCount}/{buySignalKeys.length}
                    </Typography>
                </Box>
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', py: 0.5, minHeight: 0 }}>
                    {buySignals.map((signal) => (
                        <SignalRow key={signal.key} label={signal.label} value={signal.value} signalType="buy"
                            tooltipText={signal.value === 1 ? (tooltipInfoMet[signal.key] || '') : (tooltipInfoNotMet[signal.key] || '')} />
                    ))}
                </Box>
            </Box>
            ...
            {/* Sell Section */}
            <Box sx={{ display: 'flex', flexDirection: 'column', bgcolor: alpha(theme.palette.warning.main, 0.06), borderRadius: borderRadius.sm, overflow: 'hidden' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pt: 1.5, px: 1.5 }}>
                    <Typography sx={{ fontSize: getResponsiveFontSize('md'), fontWeight: fontWeight.semibold, color: theme.palette.warning.main }}>
                        Tín hiệu Tiêu cực
                    </Typography>
                    <Typography sx={{ fontSize: getResponsiveFontSize('md'), fontWeight: fontWeight.bold, color: theme.palette.warning.main }}>
                        {sellPositiveCount}/{sellSignalKeys.length}
                    </Typography>
                </Box>
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', py: 0.5, minHeight: 0 }}>
                    {sellSignals.map((signal) => (
                        <SignalRow key={signal.key} label={signal.label} value={signal.value} signalType="sell"
                            tooltipText={signal.value === 1 ? (tooltipInfoMet[signal.key] || '') : (tooltipInfoNotMet[signal.key] || '')} />
                    ))}
                </Box>
            </Box>
