...
const LEGEND = [
    { name: 'Doanh thu', colorKey: 'info' as const },
    { name: 'Số đơn', colorKey: 'primary' as const },
];
...
const TopBrokersChart: React.FC<Props> = ({ data }) => {
    ...
    const colors = [theme.palette.info.main, theme.palette.primary.main];
    const categories = useMemo(() => data.map((d) => d.broker_name || d.broker_code), [data]);

    const series = useMemo(() => [
        { name: 'Doanh thu', type: 'bar',  data: data.map((d) => d.total_revenue) },
        { name: 'Số đơn',   type: 'line', data: data.map((d) => d.order_count) },
    ], [data]);
        ...
        yaxis: [
            {
                ...
                title: { text: 'Doanh thu', style: { fontSize: getResponsiveFontSize('xs').md, color: theme.palette.text.secondary, fontWeight: String(fontWeight.medium) } },
                labels: {
            ...
            },
            {
                seriesName: 'Số đơn',
                opposite: true,
                title: { text: 'Số đơn', style: { fontSize: getResponsiveFontSize('xs').md, color: theme.palette.text.secondary, fontWeight: String(fontWeight.medium) } },
                labels: {
        ...
        tooltip: {
            ...
            custom: ({ series: s, dataPointIndex, w }) => {
                ...
                const rows = s.map((sd: number[], si: number) => {
                    ...
                    const fmt = si === 0 ? formatCurrency(val) : `${Math.round(val)} đơn`;
    ...
    if (!data || data.length === 0) {
        return <div style={{ height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', color: theme.palette.text.secondary, fontSize: '0.875rem' }}>Chưa có dữ liệu đối tác</div>;
    }
